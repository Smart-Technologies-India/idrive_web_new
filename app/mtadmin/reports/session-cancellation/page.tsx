...
const REPORT_TITLE = "Session Cancellation Report";

const SessionCancellationPage = () => {
  ...
  const { data, isLoading } = useQuery<BookingSession[]>({
    ...
    queryFn: async () => {
      const response = await ApiCall({
        ...
        variables: { whereSearchInput: { status: "CANCELLED" } },
      });
  ...
  const columns = [
    ...
    { title: "Booking ID", key: "bookingId", render: (r: BookingSession) => r.booking?.bookingId || "N/A" },
    ...
    { title: "Car", key: "car", render: (r: BookingSession) => r.car?.carName || "N/A" },
    {
      title: "Status",
      dataIndex: "status",
      key: "status",
          ...
          <Row gutter={16} className="mb-6">
            <Col span={24}>
              <div className="bg-linear-to-r from-pink-500 to-pink-600 text-white p-4 rounded-lg">
                <Statistic
                  title={<span className="text-white text-opacity-90">Total Cancelled Sessions</span>}
                  value={data?.length || 0}
...
export default SessionCancellationPage;

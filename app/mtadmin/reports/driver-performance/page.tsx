...
const REPORT_TITLE = "Driver Performance Report";
...
const DriverPerformancePage = () => {
      ...
      const response = await ApiCall({
        query: `query GetAllBookingSession($whereSearchInput: WhereBookingSessionSearchInput!) {
          getAllBookingSession(whereSearchInput: $whereSearchInput) {
            id sessionDate status attended
            driver { id name }
          }
        }`,
  ...
  const driverStats =
    data?.reduce((acc: Record<string, DriverStat>, session) => {
      const name = session.driver?.name || "Unassigned";
      if (!acc[name]) acc[name] = { name, total: 0, completed: 0, attended: 0, cancelled: 0 };
      acc[name].total++;
      if (session.status === "COMPLETED") acc[name].completed++;
      if (session.attended) acc[name].attended++;
      if (session.status === "CANCELLED") acc[name].cancelled++;
      return acc;
    }, {}) || {};

  const tableData: DriverStat[] = Object.values(driverStats);
  ...
  const columns = [
    { title: "Driver Name", dataIndex: "name", key: "name" },
    {
      ...
      dataIndex: "total",
      key: "total",
      sorter: (a: DriverStat, b: DriverStat) => a.total - b.total,
    },
    {
      title: "Completed",
      dataIndex: "completed",
    ...
    },
    {
      ...
      key: "rate",
      render: (record: DriverStat) => {
        const rate = record.total > 0 ? ((record.attended / record.total) * 100).toFixed(1) : "0";
        return `${rate}%`;
      },
    },
  ];
          ...
          <Row gutter={16} className="mb-6">
            <Col span={12}>
              <div className="bg-linear-to-br from-indigo-500 to-indigo-600 text-white p-4 rounded-lg">
                <Statistic
                  title={<span className="text-white text-opacity-90">Total Drivers</span>}
